    ...
    render() {
        return(
            <View style={styles.container}>
                <Text style={styles.message}>Even the most precise scientific calculations, those used in the navigation of satellites and the determination of physical constants, require only 32 digits of pi to achieve more than the desired precision. These digits had been found by the early 1600s, yet the race is still on to calculate more digits of pi, with the world record being around 22 trillion digits as of 2016. This race does have some use outside of bringing fame to record-holders, since programs that calculate pi are used to test computers for bugs that might affect precision or data storage. Furthermore, the string of known digits of pi provides evidence, though not proof, that pi is a <Text style={styles.link}  onPress={ () => this._navigate('Glossary') }>normal</Text> number, since each digit seems to appear with uniform frequency. The search for more digits of pi has not only pushed mathematics forward in many cases, but may someday hold the key to distinguishing pi from other <Text style={styles.link}  onPress={ () => this._navigate('Glossary') }>irrational</Text> numbers.
                </Text>
                <Text style={styles.message}>Computers take instructions in the form of <Text style={styles.link}  onPress={ () => this._navigate('Glossary') }>algorithms</Text>, or lists of basic steps or actions. In the case of algorithms to calculate pi, the basic steps are generally arithmetic computations, with the goal of calculating one piece of an infinitely long sum or product. To find more digits of pi, the computer is simply instructed to perform the computation again, with minor changes, to calculate the next piece of the sum or product. After many <Text style={styles.link}  onPress={ () => this._navigate('Glossary') }>iterations</Text> of this computation, the computer adds together the pieces of the sum or multiplies together the pieces of the product, resulting in a value for pi that is correct to a certain number of digits.
                </Text>
